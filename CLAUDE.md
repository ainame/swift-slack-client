...
- **SlackEvent Protocol**: Base protocol that all events conform to (`Decodable, Sendable, Hashable`)
...
2. **Transforms properties** - Changes `_type` to `type`, integrates SlackBlockKit types
3. **Removes generated code** - Strips CodingKeys enums, init methods, custom decode/encode
...
- `public var _type: String?` → `public var type: String?`
- Preserves only property declarations and documentation comments
