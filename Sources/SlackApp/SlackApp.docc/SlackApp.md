...
In normal app code, prefer `import SlackAppKit`. Use `SlackApp` directly when you only want the runtime layer, or `SlackClient` when you want custom transport control or a lower-level client surface, such as Apple-platform apps using `URLSession`-based transports.
